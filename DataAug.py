...
##################################
# Input Dataset
Train = scipy.io.loadmat('GTSResizedTrain.mat')
Test = scipy.io.loadmat('GTSResizedTest.mat')
...
def GNois(img):
    img = np.array(img)
    seq = iaa.Sequential([iaa.AdditiveGaussianNoise(scale=0.15*255)])
    ImgN = seq(images=img)
    return ImgN
...
def CInt(img):
    ...
    iValue = np.ones(sizeOfIn,dtype='uint16') * np.random.randint(50,150) 
    ImgCInt += iValue
